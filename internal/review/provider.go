...
Use only the supplied ReviewInput evidence. Discussion is not mastery. An AI suggestion is not a user accomplishment. An assistant saying “done” is not proof that the user completed the work. A partial assistant turn is not a final conclusion. Every topic and accomplishment must cite existing evidence_refs from the input. The semantic array contains only topic-capable learning evidence; behavior_semantic is context for behavior and cannot support a learning topic or accomplishment. Sessions, chat and semantic snapshots can support a topic or time investment but cannot prove completion. Only completed_missions is completion evidence. Accomplishments require strong completion evidence, so prefer an empty accomplishments array and explain uncertainty in unfinished or difficulties. Never invent, alter or replace evidence references. When evidence conflicts or is weak, state uncertainty instead of guessing.`

...
| `desktop-pet` | MIT | 历史 UI Shell 改造参考 | 是（只保留并改造透明置顶与动画思路） | `https://github.com/UIU-Developers-Hub/desktop-pet` | 不随本项目重新分发其未知来源示例资产 |
...
`pet/assets/skins/studyguardian-pixel` 和 `builtin-minimal` 是本项目 Phase 6 的原创占位像素素材，使用 OpenAI ImageGen 生成于 2026-09-02，manifest 中保留了用途声明。它们不继承上表 `desktop-pet` 的示例资产授权；发布时应保留对应 manifest 和本节说明。当前产品版本的最终视觉资产状态为 `Final Visual Asset Pending`。

依赖表只记录实际 Direct Runtime Dependencies。Pet 使用 Python 标准库 + PyQt6 + `urllib.request`；Sensor 使用 `mss` + Pillow；仓库当前没有 Runtime import `requests` 或 `pywin32`。
